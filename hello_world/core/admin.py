...
# ════════════════════════════════════════════════════
# 대시보드 통계 — admin.site.index 오버라이드
# ════════════════════════════════════════════════════
...
def _patched_index(self, request, extra_context=None):
    from .models import CustomUser, Post, Group, ActivityProof, Survey
...
class MemberGradeProxy(MemberGrade):
    class Meta:
        proxy = True
        app_label = 'member'
        verbose_name = '회원 등급'
...
class UserBadgeProxy(UserBadge):
    class Meta:
        proxy = True
        app_label = 'member'
...
class UserFollowProxy(UserFollow):
    class Meta:
        proxy = True
        app_label = 'member'
        verbose_name = '팔로우'
...
class ManagementDocumentProxy(ManagementDocument):
    class Meta:
        proxy = True
        app_label = 'board'
        verbose_name = '관리 문서'
...
class ActivityProofProxy(ActivityProof):
    class Meta:
        proxy = True
        app_label = 'activity'
        verbose_name = '활동 인증'
...
class CalendarEventProxy(CalendarEvent):
    class Meta:
        proxy = True
        app_label = 'activity'
        verbose_name = '캘린더 일정'
...
class SiteConfigProxy(SiteConfig):
    class Meta:
        proxy = True
        app_label = 'operation'
        verbose_name = '사이트 설정'
...
class AdminActionLogProxy(AdminActionLog):
    class Meta:
        proxy = True
        app_label = 'operation'
...
class MemberUserAdmin(CustomUserAdmin):
    pass

@admin.register(MemberGradeProxy)
class MemberGradeProxyAdmin(MemberGradeAdmin):
    pass

@admin.register(BadgeProxy)
class BadgeProxyAdmin(BadgeAdmin):
    pass

@admin.register(UserBadgeProxy)
class UserBadgeProxyAdmin(UserBadgeAdmin):
    pass

@admin.register(UserFollowProxy)
class UserFollowProxyAdmin(UserFollowAdmin):
    pass

@admin.register(RatingProxy)
class RatingProxyAdmin(RatingAdmin):
    pass

@admin.register(BoardProxy)
class BoardProxyAdmin(BoardAdmin):
    pass

@admin.register(PostProxy)
class PostProxyAdmin(PostAdmin):
    pass
...
@admin.register(ManagementDocumentProxy)
class ManagementDocumentProxyAdmin(ManagementDocumentAdmin):
    pass
...
@admin.register(GroupMemberProxy)
class GroupMemberProxyAdmin(GroupMemberAdmin):
    pass

@admin.register(ActivityProxy)
class ActivityProxyAdmin(ActivityAdmin):
    pass

@admin.register(ActivityProofProxy)
class ActivityProofProxyAdmin(ActivityProofAdmin):
    pass

@admin.register(CalendarEventProxy)
class CalendarEventProxyAdmin(CalendarEventAdmin):
    pass

@admin.register(SurveyProxy)
class SurveyProxyAdmin(SurveyAdmin):
    pass
...
@admin.register(SiteConfigProxy)
class SiteConfigProxyAdmin(SiteConfigAdmin):
    pass

@admin.register(AdminActionLogProxy)
class AdminActionLogProxyAdmin(AdminActionLogAdmin):
    pass
...
# 👥 회원 관리
class MemberUserProxy(CustomUser):
    class Meta:
        proxy = True; app_label = 'member'
        verbose_name = '입주민'; verbose_name_plural = '입주민'

class MemberGradeProxy(MemberGrade):
    class Meta:
        proxy = True; app_label = 'member'
        verbose_name = '회원 등급'; verbose_name_plural = '회원 등급'

class BadgeProxy(Badge):
    class Meta:
        proxy = True; app_label = 'member'
        verbose_name = '배지'; verbose_name_plural = '배지'

class UserBadgeProxy(UserBadge):
    class Meta:
        proxy = True; app_label = 'member'
        verbose_name = '회원 보유 배지'; verbose_name_plural = '회원 보유 배지'

class UserFollowProxy(UserFollow):
    class Meta:
        proxy = True; app_label = 'member'
        verbose_name = '팔로우'; verbose_name_plural = '팔로우'

class RatingProxy(Rating):
    class Meta:
        proxy = True; app_label = 'member'
        verbose_name = '이웃 온기 평가'; verbose_name_plural = '이웃 온기 평가'

# 📋 게시판
class BoardProxy(Board):
    class Meta:
        proxy = True; app_label = 'board'
        verbose_name = '게시판'; verbose_name_plural = '게시판'

class PostProxy(Post):
    class Meta:
        proxy = True; app_label = 'board'
        verbose_name = '게시글'; verbose_name_plural = '게시글'

class CommentProxy(Comment):
    class Meta:
        proxy = True; app_label = 'board'
        verbose_name = '댓글'; verbose_name_plural = '댓글'

class ManagementDocumentProxy(ManagementDocument):
    class Meta:
        proxy = True; app_label = 'board'
        verbose_name = '관리 문서'; verbose_name_plural = '관리 문서'

# 🤝 소모임
class GroupProxy(Group):
    class Meta:
        proxy = True; app_label = 'group'
        verbose_name = '소모임'; verbose_name_plural = '소모임'

class GroupMemberProxy(GroupMember):
    class Meta:
        proxy = True; app_label = 'group'
        verbose_name = '소모임 회원'; verbose_name_plural = '소모임 회원'

class GroupLeaderLogProxy(GroupLeaderLog):
    class Meta:
        proxy = True; app_label = 'group'
        verbose_name = '방장 활동 기록'; verbose_name_plural = '방장 활동 기록'

class GroupDissolveVoteProxy(GroupDissolveVote):
    class Meta:
        proxy = True; app_label = 'group'
        verbose_name = '해체 투표'; verbose_name_plural = '해체 투표'

# 📅 활동/봉사
class ActivityProxyModel(Activity):
    class Meta:
        proxy = True; app_label = 'activity'
        verbose_name = '봉사 활동'; verbose_name_plural = '봉사 활동'

class ActivityProofProxy(ActivityProof):
    class Meta:
        proxy = True; app_label = 'activity'
        verbose_name = '활동 인증'; verbose_name_plural = '활동 인증'

class CalendarEventProxy(CalendarEvent):
    class Meta:
        proxy = True; app_label = 'activity'
        verbose_name = '캘린더 일정'; verbose_name_plural = '캘린더 일정'

# ⚙️ 운영
class SurveyProxy(Survey):
    class Meta:
        proxy = True; app_label = 'operation'
        verbose_name = '설문조사'; verbose_name_plural = '설문조사'

class NotificationProxy(Notification):
    class Meta:
        proxy = True; app_label = 'operation'
        verbose_name = '알림'; verbose_name_plural = '알림'

class SiteConfigProxy(SiteConfig):
    class Meta:
        proxy = True; app_label = 'operation'
        verbose_name = '사이트 설정'; verbose_name_plural = '사이트 설정'

class AdminActionLogProxy(AdminActionLog):
    class Meta:
        proxy = True; app_label = 'operation'
        verbose_name = '관리자 행위 로그'; verbose_name_plural = '관리자 행위 로그'


# ── Proxy Admin 등록 ──
@admin.register(MemberUserProxy)
class MemberUserAdmin(CustomUserAdmin): pass

@admin.register(MemberGradeProxy)
class MemberGradeProxyAdmin(MemberGradeAdmin): pass

@admin.register(BadgeProxy)
class BadgeProxyAdmin(BadgeAdmin): pass

@admin.register(UserBadgeProxy)
class UserBadgeProxyAdmin(UserBadgeAdmin): pass

@admin.register(UserFollowProxy)
class UserFollowProxyAdmin(UserFollowAdmin): pass

@admin.register(RatingProxy)
class RatingProxyAdmin(RatingAdmin): pass

@admin.register(BoardProxy)
class BoardProxyAdmin(BoardAdmin): pass

@admin.register(PostProxy)
class PostProxyAdmin(PostAdmin): pass

@admin.register(CommentProxy)
class CommentProxyAdmin(CommentAdmin): pass

@admin.register(ManagementDocumentProxy)
class ManagementDocumentProxyAdmin(ManagementDocumentAdmin): pass

@admin.register(GroupProxy)
class GroupProxyAdmin(GroupAdmin): pass

@admin.register(GroupMemberProxy)
class GroupMemberProxyAdmin(GroupMemberAdmin): pass

@admin.register(GroupLeaderLogProxy)
class GroupLeaderLogProxyAdmin(GroupLeaderLogAdmin): pass

@admin.register(GroupDissolveVoteProxy)
class GroupDissolveVoteProxyAdmin(GroupDissolveVoteAdmin): pass

@admin.register(ActivityProxyModel)
class ActivityProxyAdmin(ActivityAdmin): pass

@admin.register(ActivityProofProxy)
class ActivityProofProxyAdmin(ActivityProofAdmin): pass

@admin.register(CalendarEventProxy)
class CalendarEventProxyAdmin(CalendarEventAdmin): pass

@admin.register(SurveyProxy)
class SurveyProxyAdmin(SurveyAdmin): pass

@admin.register(NotificationProxy)
class NotificationProxyAdmin(NotificationAdmin): pass

@admin.register(SiteConfigProxy)
class SiteConfigProxyAdmin(SiteConfigAdmin):
    def has_add_permission(self, request):
        return not SiteConfigProxy.objects.exists()
    def has_delete_permission(self, request, obj=None):
        return False
    def changelist_view(self, request, extra_context=None):
        obj, _ = SiteConfigProxy.objects.get_or_create(id=1)
        from django.http import HttpResponseRedirect
        from django.urls import reverse
        return HttpResponseRedirect(reverse('admin:operation_siteconfigproxy_change', args=[obj.pk]))

@admin.register(AdminActionLogProxy)
class AdminActionLogProxyAdmin(AdminActionLogAdmin): pass
